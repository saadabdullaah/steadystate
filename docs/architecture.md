...
Sync waves establish AppProjects at `-30`, Argo configuration at `-20`, monitoring at `-18`, Rollouts at `-17`, and full-profile data namespaces/storage/cert-manager/CNPG/Barman at `-10` through `-6`. The operator remains `-10` in standard profiles and moves to `-5` in the full profile so manager discovery sees the external CRDs before registering Database child watches. Cert-manager leader election stays in its permitted `cert-manager` Namespace instead of creating default leases and RoleBindings in `kube-system`. The tenant child is `0`; inside the full profile, Team is `-1`, Database is `0`, and Application is `1`, so an Application cannot bind before its Database intent exists. The reusable demo leaf has no database binding. Standard profiles render it unchanged beside Team intent, while the full profile adds `databaseRef.name=orders` and the Database source. This preserves Phase 0–6 behavior and makes the data dependency an explicit full-profile concern. Kustomize substitutes the exact Argo source revision into `steadystate.dev/source-revision` on every rendered tenant leaf. Argo ignores controller-owned status and finalizers with `RespectIgnoreDifferences=true`. The root ignores the monitoring child as a repeated sync-wave health gate so a transient Prometheus chart health refresh cannot block an otherwise unrelated tenant commit; deployment and acceptance still explicitly require every platform child, including monitoring, to be Healthy before delivery begins.
